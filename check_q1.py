...
for path in paths:
    ...
    for i in range(2):
        ...
        if i == 0:
            cmd_0 = 'mongo disaster --quiet ' + os.getcwd() + '\\' + project_name + '\\' + 'init.js'
            print('INDEX 삭제')
            os.system(cmd_0)
        else:
        ...
        cmd_1 = 'mongo disaster --quiet --eval "var input_date = ISODate(\'2020-09-18T21:00:00Z\');" ' + path + 'q1.js'
        ...
        error_count = 0
        start = time.time()
        with open('ds2_nosql_hw2\\result_q1_1.txt') as result:
        ...
        cmd_2 = 'mongo disaster --quiet --eval "var input_date = ISODate(\'2020-09-18T23:00:00Z\');" ' + path + 'q1.js'

        print("CHECK: ", cmd_2)
        f = os.popen(cmd_2)
        error_count = 0
        start = time.time()
        with open('ds2_nosql_hw2\\result_q1_2.txt') as result:
            result_txt = f.readline().lstrip().rstrip()
            correct_txt = result.readline().lstrip().rstrip()
            if result_txt != correct_txt:
                print("ERROR: ", result_txt, correct_txt)
